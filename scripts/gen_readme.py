...
Run by the post-commit hook so the README stays true to the code on every commit. It only
...
def changelog(n: int = 12) -> str:
    # No commit hash: this file is regenerated by a POST-commit hook that folds the fresh README
    # back into the just-made commit via --amend, which changes that commit's hash. Message + author
    # date are stable across the amend, so they stay correct; a short hash would not. Dropping it is
    # what lets the changelog always include the commit currently being made (zero lag).
    try:
        out = subprocess.check_output(
            ["git", "-C", ROOT, "log", f"-{n}", "--pretty=- %s _(%ad)_", "--date=short"],
