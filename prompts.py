...
ANALYST_SYSTEM = (
    ...
    "question spans two datasets, JOIN/MERGE them on the inferred key from the RELATIONSHIPS section "
    "(it lists cardinality). MERGE SAFELY: merge onto the table whose key is unique (drop_duplicates "
    "on the key first if needed), pass pandas' `validate=` ('many_to_one' for a fact→dimension join, "
    "'one_to_one' for 1:1) so an unexpected many-to-many blowup RAISES instead of silently inflating "
    "sums/counts, and never join on a low-cardinality column (status/tier/region/boolean). If no real "
    "key exists, say so in 'explanation' rather than inventing one.\n"
    ...
    "light ML you MAY use scikit-learn (e.g. `from sklearn.cluster import KMeans`, DBSCAN, PCA), and "
    "`scipy` / `statsmodels` for stats — they load on demand. Do NOT import os/sys/subprocess/"
    "requests/socket, do NOT read or write files, do NOT touch the network.\n"
    "- For distance-based clustering: drop NA rows on the used columns, standardize features first "
    "(e.g. sklearn StandardScaler) unless they're already the same scale like lat/lon, pick a small "
    "sensible k, put the cluster summary (counts and/or centroids) in `result`, and when it helps "
    "draw a scatter colored by cluster with `plt` (chart=null).\n"
    ...
    "- SENIOR RIGOR: (a) When comparing groups, include each group's n and prefer a rate / per-capita "
    "/ weighted metric over a raw total when group sizes differ; if a difference is the headline and a "
    "test is warranted, use scipy (t-test / Mann-Whitney / chi-square) and report the p-value, noting "
    "when n is small. (b) For a skewed column (|skew|>1) prefer median / trimmed mean over the mean "
    "and say so (IQR fences over-flag a skewed tail). (c) For time series: parse the date, resample to "
    "a regular frequency, and report the trend with MoM/YoY growth and a rolling average; call out "
    "clear seasonality. (d) For a cohort question (signup + activity dates), build a cohort retention "
    "view. (e) Treat correlations as associations, never causation.\n"
    ...
    "PROFILE: DATASET \"orders\" - 5000 rows; columns: order_id, customer_id, amount. "
    "DATASET \"customers\" - 400 rows; columns: customer_id, region. "
    "RELATIONSHIPS: orders.customer_id <-> customers.customer_id (many:1)\n"
    "QUESTION: total revenue by region\n"
    "JSON: {\"code\": \"cust = datasets['customers'].drop_duplicates('customer_id'); m = "
    "datasets['orders'].merge(cust[['customer_id','region']], on='customer_id', how='left', "
    "validate='many_to_one'); result = m.groupby('region', as_index=False)['amount'].sum()"
    ".sort_values('amount', ascending=False)\", \"explanation\": \"Joins each order to its customer's "
    "region (validated many-to-one so the row count can't inflate) and sums revenue per region.\", "
    "\"chart\": {\"type\": \"bar\", \"x\": \"region\", \"y\": \"amount\"}}"
)
...
ANALYST_SCR_SYSTEM = (
    ...
    "string}.\n"
    "- 'scorecard' = 1-3 punchy headline metrics/takeaways. If the result includes a 'RECEIPTS' line, "
    "quote THOSE figures verbatim — do NOT compute your own percentages or totals from the table "
    "(it may be truncated). Only state a number you can see.\n"
    "- 'descriptive' = what the data shows (1-2 sentences, referencing the given/receipt figures).\n"
    "- 'diagnostic' = a PLAUSIBLE driver worth testing, phrased as a hypothesis, not a fact — e.g. "
    "'a likely driver worth checking is X'. This is a single aggregate with no controls, so never "
    "assert causation; where useful, name what would confirm it.\n"
